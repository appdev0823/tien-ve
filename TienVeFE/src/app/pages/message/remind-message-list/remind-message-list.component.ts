...
import { RemindMessageDTO, RemindMessageSearchQuery } from 'src/app/dtos';
...
@Component({
    selector: 'app-remind-message-list',
    templateUrl: './remind-message-list.component.html',
    styleUrls: ['./remind-message-list.component.scss'],
})
export class RemindMessageListComponent extends PageComponent implements OnInit, OnDestroy {
    ...
    public dataList: RemindMessageDTO[] = [];
    public dataTotal = 0;
    ...
    constructor(private _remindMessage$: RemindMessageService) {
        super();
    }
    ...
    private _getList() {
        const params: RemindMessageSearchQuery = {};
        ...
        const sub = this._remindMessage$.getList(params).subscribe((res) => {
    ...
    private async _deleteMultiple() {
        ...
        const result = await this._remindMessage$.deleteMultiple(idList);
        if (!result.isSuccess) {
    ...
    public viewDetail(idx: number) {
        ...
        const modal = this.modal$.open(RemindMessageDetailComponent, { centered: true });
        const cmpIns = modal.componentInstance as RemindMessageDetailComponent;
